<script>
	...
	async function fetchAgents() {
		...
		error = '';
		try {
			const response = await fetch('/api/up-rera-agents');
			...
			error = e.message || 'Failed to fetch UP RERA agents.';
		} finally {
			loading = false;
		}
	}
	...
	async function startScraping() {
		...
		scrapeMessage = '';
		try {
			const response = await fetch('/api/up-rera-agents?refresh=true');
			const result = await response.json();
			if (result.success) {
				agents = result.data;
				if (result.scrapeError) {
					scrapeMessage = `Scraping failed (${result.scrapeError}), showing cached data.`;
				} else {
					scrapeMessage = `Scraping complete! ${result.total || agents.length} agents in database.`;
				}
			} else {
...
<div class="container mx-auto p-6 space-y-6">
	...
	<div class="flex items-center justify-between">
		<div>
			<h1 class="text-2xl font-bold text-gray-900">UP RERA Agents</h1>
			<p class="text-sm text-gray-500 mt-1">Official registered real estate agents in Uttar Pradesh</p>
		</div>
		<button
