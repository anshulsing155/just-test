<script>
	...
	function startPolling() {
		...
		pollTimer = setInterval(async () => {
			try {
				const res = await fetch('/api/telangana-rera-agents?action=scrape-status');
				...
				if (data.success) {
					...
					if (!progress.running && progress.done > 0) {
						stopPolling();
						scraping = false;
						scrapeMsg = `Done! ${progress.done} agents scraped.`;
						await fetchAgents();
					}
				}
			} catch {}
	...
	async function fetchAgents() {
		loading = true; error = '';
		try {
			const res    = await fetch('/api/telangana-rera-agents');
	...
	async function openBrowser() {
		readyMsg = ''; error = '';
		try {
			const res    = await fetch('/api/telangana-rera-agents?action=open-browser');
			const result = await res.json();
			if (result.success) {
				readyMsg = 'Browser opened with "Agent" pre-selected. Submit the search form, then click "Check Ready".';
			} else throw new Error(result.error);
		} catch (e) {
	...
	async function checkReady() {
		...
		try {
			const res    = await fetch('/api/telangana-rera-agents?action=check-ready');
			const result = await res.json();
	...
	async function scrapeNow() {
		...
		try {
			const res    = await fetch('/api/telangana-rera-agents?refresh=true');
			const result = await res.json();
			if (result.success) {
				agents    = result.data || [];
				if (result.scraping) {
					...
					scraping  = false;
					scrapeMsg = `Done! ${result.total ?? agents.length} agents scraped.`;
				}
			} else throw new Error(result.error);
		} catch (e) {
			scraping = false;
	...
	async function closeBrowser() {
		await fetch('/api/telangana-rera-agents?action=close-browser').catch(() => {});
	...
	function getName(a)  { return a.agentName || a.name || '—'; }

	function getFlatFields(agent) {
		const raw    = agent?.rawData;
		const detail = raw?.detail;
		if (!detail?.sections) return [];
		const pairs  = [];
		const seen   = new Set();
		Object.entries(detail.sections).forEach(([, fields]) => {
	...
	function getDetailTables(agent) {
		return agent?.rawData?.detail?.tables || [];
	}
...
<svelte:head>
	<title>Telangana RERA Agents — RERA Toolkit</title>
</svelte:head>
...
<div class="space-y-6">
	...
	<div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
		<div class="flex flex-wrap items-start justify-between gap-4">
			<div>
				<h1 class="text-2xl font-bold text-slate-900">Telangana RERA — Agents</h1>
				<p class="mt-1 text-sm text-slate-500">
					Registered agents from RERAIT — Telangana Real Estate Regulatory Authority.
				</p>
			</div>
			...
			<div class="flex flex-wrap gap-2">
				<button
					...
					title="Open Chrome — Agent radio is pre-selected"
				>
		...
		{:else if scraping}
			<div class="mt-4 flex items-center gap-3 rounded-xl border border-indigo-200 bg-indigo-50 px-4 py-3 text-sm text-indigo-800" transition:slide>
				...
				<span><strong>Collecting agent rows…</strong> Building the list before opening detail pages.</span>
			</div>
		{/if}
	...
	<div class="rounded-2xl border border-blue-100 bg-blue-50 p-5 text-sm text-blue-800">
		<p class="font-semibold mb-2">How to scrape Telangana RERA Agents:</p>
		<ol class="list-decimal list-inside space-y-1 text-blue-700">
			<li>Click <strong>Open Browser</strong> — Chrome opens with the "Agent" radio pre-selected.</li>
			<li>Fill in any search criteria (leave blank for all agents) and submit the form.</li>
			<li>Click <strong>Check Ready</strong> — once the agents table appears, status shows "Ready".</li>
			<li>Click <strong>Scrape Now</strong> — all rows are collected, then "View Details" is clicked for each agent.</li>
			<li>Progress updates automatically. The browser closes when scraping finishes.</li>
		</ol>
	</div>
	...
	<div class="flex items-center justify-between gap-3">
		...
		<input
			type="text"
			placeholder="Search agents, district, cert no…"
		...
		{:else if paginated.length === 0}
			<div class="flex flex-col items-center justify-center py-20 text-center">
				...
				<p class="mt-4 text-sm font-medium text-slate-500">
					{searchTerm ? 'No agents match your search.' : 'No data yet — open the browser, submit a search, then scrape.'}
				</p>
			</div>
	...
	>
		...
		>
			...
			<div class="flex shrink-0 items-start justify-between border-b border-slate-100 p-6">
				<div>
					<h2 class="text-lg font-bold text-slate-900">{getName(modalAgent)}</h2>
					<p class="mt-0.5 font-mono text-xs text-slate-500">{modalAgent.reraRegNo || ''}</p>
				</div>
				<button
					on:click={closeModal}
			...
			<div class="flex-1 overflow-y-auto p-6 space-y-6">
				...
				<div class="grid grid-cols-2 gap-2 sm:grid-cols-3">
					{#each [
						...
						['Mobile',         modalAgent.mobile],
						['Email',          modalAgent.email],
						['Address',        modalAgent.address]
					] as [label, val]}
						{#if val && val !== '—' && val !== modalAgent.reraRegNo || label === 'Certificate No'}
				...
				<!-- Dynamic fields from detail page -->
				{#if getFlatFields(modalAgent).length > 0}
					<div>
						<h3 class="mb-3 text-sm font-bold text-slate-700">Full Detail (all captured fields)</h3>
						<div class="grid grid-cols-1 gap-1.5 sm:grid-cols-2">
							{#each getFlatFields(modalAgent) as [key, val]}
				...
				<!-- Dynamic tables -->
				{#each getDetailTables(modalAgent) as tbl, ti}
					{#if tbl.rows && tbl.rows.length > 0}
						<div>
							<h3 class="mb-2 text-sm font-bold text-slate-700">
								{tbl.section || 'Table'} {getDetailTables(modalAgent).length > 1 ? `(${ti + 1})` : ''}
							</h3>
